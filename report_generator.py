...
# Volaris-inspired visual palette.
# This is an approximation based on the supplied reference report,
# not an official Volaris template or trademark asset.
...
# Stable keys are deliberately ASCII/simple so Streamlit and Python
# imports are not affected by accents, punctuation or spaces.
SECTION_CONFIG = {
    ...
    "rubstrip": {
        "title": "FWD AND AFTER (RUBSTRIP PANEL, ACOUSTIC SEGMENT)",
        "layout": "standard",
    },
    ...
    "tarp": {
        "title": "PLASTIC TARP INSIDE-LOWER AREA AND DESICCANTS",
        "layout": "standard",
    },
...
def _add_header(slide, sn, section=None):
    ...
    if section:
        _text(slide, section, 0.38, 0.58, 12.55, 0.38, size=16, bold=True)
...
def _add_final_slide(prs, sn, qc, report_date, page_no):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    _add_header(slide, sn, "REPORT COMPLETED")

    _text(
        slide,
        "REPORT ACCOMPLISHED BY QC",
        0.7,
        2.25,
        5.0,
        0.4,
...
def generate_report(pn, sn, qc, report_date, uploads, discrepancies=None):
    if not str(sn).strip():
        raise ValueError("Serial Number (SN) is required.")
    ...
    _add_final_slide(prs, sn, qc, report_date, page_no)

    output = OUTPUT_DIR / f"INCOMING_{sn}.pptx"
    prs.save(output)
    return output
